...
export async function getIntelligenceMetrics(organizationId: string) {
  ...
  const recovered = await db.revenueEvent.aggregate({
    where: { organizationId, type: "reactivated_won" },
    _sum: { amount: true },
  });

  const chat = await db.chatSession.aggregate({
    where: { organizationId },
    _count: true,
  });
  const chatLeads = await db.chatSession.count({
    where: { organizationId, leadId: { not: null } },
  });
  const handoffs = await db.chatSession.count({
