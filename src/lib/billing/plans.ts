...
export const PLANS: PlanDefinition[] = [
  ...
  },
  {
    ...
    features: [
      ...
      "Paddle billing + custom contract",
    ],
  },
];
...
export async function getBillingUsage(organizationId: string) {
  ...
  const [activeLeads, seatsUsed, automations, whatsappSent] = await Promise.all([
    db.lead.count({
    ...
    Promise.resolve(subscription?.whatsappSentThisPeriod ?? 0),
  ]);
