...
async function limitAuth(extra: string) {
  const h = await headers();
  const ip = h.get("x-forwarded-for")?.split(",")[0]?.trim() || h.get("x-real-ip") || "local";
  return rateLimit(`auth:${ip}:${extra}`, "auth");
}

export async function loginAction(formData: FormData): Promise<ActionResult<{ redirectTo: string }>> {
  const email = String(formData.get("email") ?? "").toLowerCase().trim();
  const limited = await limitAuth(email || "login");
  if (!limited.ok) return fail("Too many requests. Try again shortly.");
  try {
    ...
    if (error instanceof AuthError) {
      logSecurity("auth.failure", { action: "login" });
...
export async function requestPasswordResetAction(formData: FormData): Promise<ActionResult<{ resetUrl?: string }>> {
  const limited = await limitAuth("reset");
  if (!limited.ok) return fail("Too many requests. Try again shortly.");
  const email = String(formData.get("email") ?? "").toLowerCase().trim();
...
export async function resetPasswordAction(formData: FormData): Promise<ActionResult> {
  const limited = await limitAuth("reset-complete");
  if (!limited.ok) return fail("Too many requests. Try again shortly.");
...
export async function acceptInviteAction(formData: FormData): Promise<ActionResult> {
  try {
    const limited = await limitAuth("invite");
    if (!limited.ok) return fail("Too many requests. Try again shortly.");
    const token = String(formData.get("token") ?? "");
